namespace WebHooks.Functions.Account
{
    ...
    using WebHooks.BusinessLogic.Services.Account;
    using WebHooks.BusinessLogic.Services.Data;

    public class DoSomethingUponUpdate : DataverseWebHook
    {
        #region Constructors

        public DoSomethingUponUpdate(ICrmService crmService, IOrganizationService organizationService, ILoggerFactory loggerFactory) 
            : base(organizationService, loggerFactory)
        {
            ...
            PluginMessageName = PlatformMessageHelper.Update;
        ...
        [FunctionName("Account_DoSomethingUponUpdate")]
        public async Task<HttpResponseMessage> Run(
            [HttpTrigger(AuthorizationLevel.Function, "post", Route = "web-hooks/account/do-something-upon-update")] HttpRequestMessage req)
        {
        ...
        protected override void Validate()
        {
            ...
            if (RemoteExecutionContext.MessageName != PlatformMessageHelper.Update)
            {
                throw new InvalidPluginExecutionException(string.Format(CultureInfo.InvariantCulture,
                    TraceMessageHelper.IncorrectMessageName, UnderlyingSystemTypeName,
                    PlatformMessageHelper.Update, RemoteExecutionContext.MessageName));
            }
