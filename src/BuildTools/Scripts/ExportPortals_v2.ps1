...
.\ExportPortal_v2.ps1 -EnvironmentName $EnvironmentName -PortalName "$env:PowerApps_CP_PortalName" -WebSiteId "$env:PowerApps_CP_WebsiteId" -Url "$Url" -TenantId "$TenantId" -ServicePrincipalId "$ServicePrincipalId" -ServicePrincipalSecret "$ServicePrincipalSecret" -AsPlainText
