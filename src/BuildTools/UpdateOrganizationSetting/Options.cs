namespace Aylos.Xrm.Sdk.BuildTools.UpdateOrganizationSetting
{
    ...
    public class Options : CommandLineOptions
    {
        ...
        [Option("enableAccessToLegacyWebClientUI",
            Required = false,
            DefaultValue = false,
            HelpText = "Boolean value determines if the legacy web client is enabled.")]
        public bool? EnableAccessToLegacyWebClientUI { get; set; }

        [Option("legacyFormRendering",
            Required = false,
            DefaultValue = false,
            HelpText = "Boolean value determines if the legacy form rendering is enabled.")]
        public bool? LegacyFormRendering { get; set; }

        [Option("organizationName",
            Required = true,
            HelpText = "The name of the organization.")]
        public string OrganizationName { get; set; }

        [Option("sessionTimeoutEnabled",
            Required = false,
            DefaultValue = false,
            HelpText = "Boolean value determines if the session timeout is enabled.")]
        public bool? SessionTimeoutEnabled { get; set; }

        [Option("sessionTimeoutInMinutes",
            Required = false,
            DefaultValue = 1440,
            HelpText = "The number of minutes before the session timeouts.")]
        public int SessionTimeoutInMinutes { get; set; }

        [Option("sessionTimeoutReminderInMinutes",
            Required = false,
            DefaultValue = 20,
            HelpText = "The number of minutes of the timeout reminder before the session timeouts.")]
        public int SessionTimeoutReminderInMinutes { get; set; }

        [Option("slaPauseStates",
            Required = false,
            DefaultValue = "<entitiesstates><entity value=\"incident\"/></entitiesstates>",
            HelpText = "A formatted XML text containing the SLA pause states per entity.")]
