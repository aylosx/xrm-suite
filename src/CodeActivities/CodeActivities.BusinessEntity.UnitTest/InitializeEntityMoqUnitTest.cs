namespace CodeActivities.BusinessEntity.UnitTest
{
    ...
    using Aylos.Xrm.Sdk.CodeActivities.MoqTests;
    ...
    [TestClass]
    public class InitializeEntityMoqUnitTest : CustomCodeActivityUnitTest<InitializeEntity>
    {
        ...
        Mock<CrmService> _crmServiceMock;
        ...
        {
            var cscMock = new Mock<CrmServiceContext>(CurrentUserService).SetupAllProperties();
            CustomCodeActivity.OrganizationServiceContext = cscMock.Object;

            _crmServiceMock = new Mock<CrmService>(CustomCodeActivity.OrganizationServiceContext, TracingService).SetupAllProperties();
            CustomCodeActivity.CrmService = _crmServiceMock.Object;

            var iesMock = new Mock<InitializeEntityService>(CustomCodeActivity.CrmService, CustomCodeActivity.OrganizationServiceContext, WorkflowContext, TracingService).SetupAllProperties();
            CustomCodeActivity.InitializeEntityService = iesMock.Object;
        }
        ...
        private void SetupMockResponseForAccount(Guid key, string accountNumber)
        {
            ...
            if (key == _accountKey) {
                _crmServiceMock.Setup(x => x.GetAccountByKey(_accountKey)).Returns(account);
            }
            else {
                _crmServiceMock.Setup(x => x.GetAccountByKey(It.IsAny<Guid>())).Returns((Account)null);
            }
